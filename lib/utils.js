 ...
 * Known catalog data wins; otherwise observed request bounds are shown, e.g.
 ...
 * "This model's maximum context length is 128000 tokens" (OpenAI) or
 * "prompt is too long: 216102 tokens > 200000 maximum" (Anthropic).
 ...
 * Checks the response headers (x-ratelimit-reset, retry-after) and the parsed
 * JSON error body (error.metadata.headers["X-RateLimit-Reset"]). Returns null
 * when the provider didn't communicate a reset time.
...
export function extractRateLimitResetMs(text, headerGet) {
  ...
  if (text) {
    try {
      ...
      const metaHeaders = data && data.error && data.error.metadata && data.error.metadata.headers
      if (metaHeaders && typeof metaHeaders === 'object') {
        const reset = metaHeaders['X-RateLimit-Reset'] ?? metaHeaders['x-ratelimit-reset'] ?? metaHeaders['X-Ratelimit-Reset']
...
export function isModelEligibleForRouting(r) {
  ...
  // (b) sends the next auto-fastest request straight back into the same 429. Credit
