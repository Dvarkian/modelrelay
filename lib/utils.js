...
import { TAG_VOCABULARY, getModelTags } from '../tags.js'
...
export function filterModelsByRequested(results, requestedModel, canonicalizeFn) {
  ...
  if (requested.startsWith(TAG_REQUEST_PREFIX)) {
    const tagName = requested.slice(TAG_REQUEST_PREFIX.length)
    if (!TAG_VOCABULARY.includes(tagName)) return []
    return results.filter(r => getModelTags(r.modelId).includes(tagName))
  }
