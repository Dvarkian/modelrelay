 ...
 * Example: Google's "Content cannot be a plain string" — these are
 * multimodal-only models (image/video gen) that do support a valid
 * HTTP response but reject chat completions with a 400.
 */
export function isIncompatibleModelError(errorText, status) {
  const t = String(errorText || '')
  return /content cannot be a plain string|model does not support text input/i.test(t)
}
...
export function parseMaxTokensCapFromError(errorText) {
  const t = String(errorText || '')
  const m = t.match(/max_?tokens[^\d]{0,60}must be (?:less than or equal to|no more than|at most)[^\d]{0,12}([\d,.]+)/i)
  if (!m) return null
