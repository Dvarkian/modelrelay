...
export function extractDuckAiModels(payload) {
  ...
  return records.flatMap(record => {
    const modelId = typeof record === 'string' ? record.trim() : asText(record?.id || record?.model || record?.integration_id || record?.name);
...
function extractTextFromObject(value) {
  ...
  return asText(value.message || value.response || value.text || value.content || value.chunk)
    || asText(value?.choices?.[0]?.message?.content) || asText(value?.choices?.[0]?.text);
}
...
export class DuckAiClient {
  ...
  async bootstrap() {
    ...
    this.bootstrapPromise = (async () => {
      ...
      try {
        const response = await this.fetch(`${this.origin}${STATUS_PATH}`, { method: 'GET', headers: { Accept: 'application/json', 'User-Agent': USER_AGENT, ...(this.cookies ? { Cookie: this.cookies } : {}) }, signal: controller.signal });
  ...
  async discoverModels() {
    ...
    const models = extractDuckAiModels(parseJsonSafely(bootstrap.body));
