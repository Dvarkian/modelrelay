...
// provider is always registered with these six currently available models.
const DUCKAI_MODELS = [
...
async function solveDuckVqdChallenge(challenge) {
    ...
    this.bootstrapPromise = (async () => {
      ...
      try {
        const response = await this.fetch(`${this.origin}${STATUS_PATH}`, {
          method: 'GET',
          headers: { accept: '*/*', 'x-vqd-accept': '1', 'user-agent': USER_AGENT, ...(this.cookies ? { cookie: this.cookies } : {}) },
        ...
        this.vqd = await solveDuckVqdChallenge(challenge);
